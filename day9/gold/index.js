...
const ropes = [
    { id: "H", x: 500, y: 500, prio: 0 },
    { id: "1", x: 500, y: 500, prio: 1 },
    { id: "2", x: 500, y: 500, prio: 2 },
    { id: "3", x: 500, y: 500, prio: 3 },
    { id: "4", x: 500, y: 500, prio: 4 },
    { id: "5", x: 500, y: 500, prio: 5 },
    { id: "6", x: 500, y: 500, prio: 6 },
    { id: "7", x: 500, y: 500, prio: 7 },
    { id: "8", x: 500, y: 500, prio: 8 },
    { id: "9", x: 500, y: 500, prio: 9 }
]


ropes.forEach((rope) => overlayMap[rope.x][rope.y] = rope.id )

traceMap[startPosition.x][startPosition.y] = "#"

const updateOverlayMap = () => {
    const head = ropes[ropes.length -1]
    
    for (let i = head.x - 70; i < head.x + 70; i++) {
        for (let j = head.y - 70; j <  head.y + 70; j++) {
            overlayMap[i][j] = ".";
        }
    }
...
const shouldIMoveAndWhere = (currentRope, nextRope) => {
    if(
        currentRope.x === nextRope.x &&
        currentRope.y === nextRope.y
    ) return "S"
    ...
    for (let x = currentRope.x -2; x <= currentRope.x + 2; x++) {
        for (let y = currentRope.y - 2; y <= currentRope.y + 2; y++) {
            if(overlayMap[x][y] && overlayMap[x][y] === nextRope.id) {
                ...
                else if (x === currentRope.x && y === currentRope.y - 2 ) newMovement = "L"
                else if (y === currentRope.y && x === currentRope.x + 2 ) newMovement = "D"
                else if (y === currentRope.y && x === currentRope.x - 2 ) newMovement = "U"

                if ( x === currentRope.x + 2 && y < currentRope.y ) newMovement = "DL"
                else if ( x === currentRope.x + 2 && y > currentRope.y ) newMovement = "DR"
                else if(x === currentRope.x - 2 && y < currentRope.y ) newMovement = "UL"
                else if(x === currentRope.x - 2 && y > currentRope.y ) newMovement = "UR"
                else if(y === currentRope.y + 2 && x < currentRope.x ) newMovement = "UR"
                else if(y === currentRope.y - 2 && x > currentRope.x ) newMovement = "DL"
                else if( y === currentRope.y -2 && x < currentRope.x) newMovement = "UL"
                else if( y === currentRope.y +2 && x > currentRope.x) newMovement = "DR"
