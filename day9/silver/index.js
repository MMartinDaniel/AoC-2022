    ...
    for (let x = tailPosition.x -2; x <= tailPosition.x + 2; x++) {
        for (let y = tailPosition.y - 2; y <= tailPosition.y + 2; y++) {
            if(overlayMap[x][y] && overlayMap[x][y] === "H") {
                ...
                else if (x === tailPosition.x && y === tailPosition.y - 2 ) newMovement = "L"
                else if (y === tailPosition.y && x === tailPosition.x + 2 ) newMovement = "D"
                else if (y === tailPosition.y && x === tailPosition.x - 2 ) newMovement = "U"

                if ( x === tailPosition.x + 2 && y < tailPosition.y ) newMovement = "DL"
                else if ( x === tailPosition.x + 2 && y > tailPosition.y ) newMovement = "DR"
                else if(x === tailPosition.x - 2 && y < tailPosition.y ) newMovement = "UL"
                else if(x === tailPosition.x - 2 && y > tailPosition.y ) newMovement = "UR"
                else if(y === tailPosition.y + 2 && x < tailPosition.x ) newMovement = "UR"
                else if(y === tailPosition.y - 2 && x > tailPosition.x ) newMovement = "DL"
                else if( y === tailPosition.y -2 && x < tailPosition.x) newMovement = "UL"
                else if( y === tailPosition.y +2 && x > tailPosition.x) newMovement = "DR"
